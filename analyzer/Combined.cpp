...
class DFF {
    ...
    static bool exists(InstFact* dff_in, string key)
    {
        return dff_in->count(key) != 0;
    }
...
namespace{
    struct ConstantProp : public FunctionPass
    {
        ...
        bool flowFunction(Instruction* inst, InstFact* dff_in)
        {  
            // the first thing we need to do is check to see if it's a PHI node
            if(PHINode *PN = dyn_cast<PHINode>(inst))
            {
                errs() << "ERROR: We shouldn't have a PHI node if worklist is running correctly.\n";
                return false;
            }
            ...
            for (User::op_iterator i = inst->op_begin(), e = inst->op_end(); i != e; ++i) 
            {
                ...
                if (!Op)
                {
                    ...
                    string idx = (string) (*i)->getName();
                    errs() << " the string index is: " << idx << ".\n";
                    if(DFF<int>::exists(dff_in, idx))
                    {
                        errs() << idx << " exists in the instruction fact, so let's check if it has a value.\n";
                        if( (*dff_in)[idx].size() == 1 )
                        {
                            errs() << "getting the iterator\n";
                            DFF<int>::SET_IT it = (*dff_in)[idx].begin();
                            int val = *it;
                            ...
                            errs() << "&&&&& newConst is: " << *newConst << ".\n";

                            Constant* myConst = dyn_cast<Constant>(newConst);
                            errs() << "&&&&& myConst is: " << *myConst << ".\n";
                            ...
                            errs() << "!!!!!!!!!! This item was in the set, it's value: " << val << ". We will no push it onto the Ops array.\n";
                            ...
                            return false; // we don't have any info for this instruction, but it exists in the map - ignore/weird case
                        }
                    } else
                    {                     
...
//                    if(Constant *newOp = dyn_cast<Constant>(*i))
//                    {
                        if (ConstantExpr *NewCE = dyn_cast<ConstantExpr>(Op))
                        {
                            errs() << "######### Got a constant expression, the vallue is: " << NewCE << ". Will we handle this case???\n";
                            return false; //TODO look at next line
                            //Op = constantFold(NewCE, DL, TLI); // this is what the original code does.. 
                        }
//                    }
                }
errs() << "the type is: " << *Op->getType();                
            ...
            errs() << "doing the DFF stuff\n"; 

            // we need to update this in the DFF, but first we want to make a temporary one 
            // so that we can compare the modified with the original to see if it's changed
            InstFact* tempIF = dataFlowFactsMap.getTempFact((string) inst->getName());
            ...
            errs() << "the int value we will store is: " << inputInt << ".\n";

            // now that we know the variable we kill the original
            // and insert it into the temp map
            DFF<int>::removeVarFacts(tempIF, varID);
            DFF<int>::setVarFact(tempIF, varID, inputInt);

            if(DFF<int>::exists(tempIF, varID))
            {
                errs() << "the variable " << varID << " is in the map, so we are good.\n"; 
            } else {
                errs() << "ERROR: why wasn't this added????\n";
            }

            return dataFlowFactsMap.setInsFact((string) inst->getName(), tempIF);
        }
        ...
        void setBottom(string inst)
        {   
            dataFlowFactsMap.setFactFullSet(inst);
        }   

        void setTop(string inst)
        {  
        ...
        void runWorkList(Function &F){
            ...
            errs() << "RUNNING THE WORKLIST FOR CONSTANT PROP!\n";
            errs() << "populating the BB queue!\n";
            populateBBQueue(F);
            errs() << "done populating the BB queue!\n";
            ...
            while (!bbQueue.empty()){
                ...
                for (BasicBlock::iterator I = BB->begin(), IE = BB->end(); I != IE; I++){
                    ...
                    dataFlowFactsMap.printInsFact((string) I->getName());
                }
        ...
        InstFact* getBBInstFactOut(BasicBlock *BB){
            ...
            return dataFlowFactsMap.getInsFact((string) I->getName());
        }
    };
}
